            ...
            <div class="flex flex-auto justify-between">
                ...
                <div class="hidden lg:ml-6 lg:flex lg:space-x-12">
                    <a href="{{ route('welcome.index') }}"
                        class="text-[#1E2B4F] relative font-semibold inline-flex items-center px-1 text-lg {{ request()->routeIs('welcome.index') ? 'active' : '' }}">
                        Home
                    </a>
                    <a href="{{ route('about.index') }}"
                        class="text-[#1E2B4F] relative hover:text-gray-500 inline-flex items-center px-1 pt-1 text-lg font-medium {{ request()->routeIs('about.index') ? 'active' : '' }}">
                        About
                    </a>
                    <a href="{{ route('diagnosa.index') }}"
                        class="text-[#1E2B4F] relative hover:text-gray-500 inline-flex items-center px-1 pt-1 text-lg font-medium {{ request()->routeIs('diagnosa.index') ? 'active' : '' }}">
                        Diagnosa
                    </a>
                    <a href="{{ route('edukasi.index') }}"
                        class="text-[#1E2B4F] relative hover:text-gray-500 inline-flex items-center px-1 pt-1 text-lg font-medium {{ request()->routeIs('edukasi.index') ? 'active' : '' }}">
                        Edukasi
                    </a>
                </div>
                ...
                <div class="hidden lg:ml-10 lg:flex lg:items-center border-l pl-4 pt-2">
                    <div x-data="{ profileDekstopOpen: false }" class="ml-3 relative">
                        ...
                        <div x-show="profileDekstopOpen" @click.outside="profileDekstopOpen = false"
                            ...
                            <a class="block px-4 py-2 text-sm text-[#1E2B4F] hover:bg-gray-100" role="menuitem"
    ...
    <div class="lg:hidden" id="mobile-menu" x-show="navbarMobileOpen">
        <div class="pt-2 pb-3 space-y-1">
          ...
          -->
            ...
            <a href="{{ route('about.index') }}"
                class="border-transparent text-[#1E2B4F] hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700 block pl-3 pr-4 py-2 border-l-4 text-base font-medium">About</a>
            <a href="{{ route('diagnosa.index') }}"
                class="border-transparent text-[#1E2B4F] hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700 block pl-3 pr-4 py-2 border-l-4 text-base font-medium">Diagnosa</a>
            <a href="{{ route('edukasi.index') }}"
                class="border-transparent text-[#1E2B4F] hover:bg-gray-50 hover:border-gray-300 hover:text-gray-700 block pl-3 pr-4 py-2 border-l-4 text-base font-medium">Edukasi</a>
        </div>
        ...
        @auth
            <div x-data="{ profileMobilenOpen: false }" class="pt-4 pb-3 border-t border-gray-200">
                <div @click="profileMobilenOpen = ! profileMobilenOpen" class="flex items-center px-4 cursor-pointer">
                    ...
                    <div class="ml-3">
                        <div class="text-base font-medium text-[#1E2B4F]">Hi, {{ Auth::user()->name }}</div>
                        <div class="text-sm text-[#AFAEC3]">{{ Auth::user()->role }}</div>
                    </div>
                </div>
